// components/ui/molecules/DiscountBanner.tsx
import React from "react";
import { CTA } from "@/components/ui/atoms/CTA";

type DiscountBannerProps = {
    headline?: string;            // NEW: override banner heading
    discountLabel?: string;       // e.g. "50%"
    discountedPrice?: string;     // e.g. "$12.50"
    originalPrice?: string;       // e.g. "$25"
    ctaLabel?: string;
    helperText?: string;
    className?: string;
    onClick?: () => void;
    ...
    return (
        <section
            className={[
                ...
                "rounded-[12px] border bg-brand-purple-2",
                "border-brand-migrate-mate",
                "self-stretch w-full",
                className,
            ].join(" ")}
